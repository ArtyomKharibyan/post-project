  ...
  const getPost = useCallback(async (isFetch?: boolean) => {
    try {
      if (profileId) {
        ...
        if (isFetch) {
          setPostData(prevState => [...prevState, ...response.data.posts]);
        } else {
  ...
  return (
    <div>
      <div>
        ...
        <InfiniteScroll className="flex-col flex justify-center" dataLength={postData.length} next={fetchMoreData}
          hasMore={hasMore} loader={<p className="flex w-full justify-center">Loading...</p>}>
