...
const DeleteModal: FC<DeleteModalProps> = ({ showDeleteModal, setShowDeleteModal, setPostData, selectedPost }) => {
	...
	const handleDeletePost = async (postToDelete: Post | Post[]): Promise<void> => {
		try {
			if (Array.isArray(postToDelete)) {
				const deleteRequests = postToDelete.map(post => axios.delete(`${Api_Url}/post/${post.id}`));
				await Promise.all(deleteRequests);
				setPostData(prevPostData =>
					prevPostData.filter(post => !postToDelete.some(postToDeleteItem => postToDeleteItem.id === post.id))
				);
			} else {
